...
function EditModal() {
  ...
  const { mutateAsync } = useApiMutation(`/expense/category/${id}`, "patch");
  const dataFromCache: Array<Expense.IExpenseCategory> = useGetDataFromCache([
    "expense-category",
  ]);
  const foundItem = dataFromCache?.find(
    (item: Expense.IExpenseCategory) => item._id === id
  );
