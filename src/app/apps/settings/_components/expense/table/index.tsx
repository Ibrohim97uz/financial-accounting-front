...
function TableComponent({
  data,
}: Expense.IExpenseTable<Expense.IExpenseCategory>) {
  const { t } = useTranslation();
  const defaultValue = { open: false, data: null, type: null };
  const [dialog, setDialog] =
    useState<Components.IDialog<Expense.IExpenseCategory>>(defaultValue);
    ...
    >
      ...
      {data?.length ? (
        ...
        <EmptyWindow
          title={t("Bo’sh oyna")}
          description={t(
            "Bu yerda xarjatlar kategoriyasini ko'rishingiz mumkin"
          )}
        />
      )}
