...
function TableComponent({ data }: Expense.IExpenseTable<Expense.IExpense>) {
  const { t } = useTranslation();
  const defaultValue = { open: false, data: null, type: null };
  const [dialog, setDialog] =
    useState<Components.IDialog<Expense.IExpense>>(defaultValue);
  ...
  return (
    ...
    >
      ...
      {data?.length ? (
        ...
        <EmptyWindow
          title={t("Bo’sh oyna")}
          description={t("Bu yerda xarjatlar ro'yxatini ko'rishingiz mumkin")}
        />
      )}
