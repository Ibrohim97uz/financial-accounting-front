...
function TableComponent({ data }: Income.IIncomeTable<Income.IIncome>) {
  const { t } = useTranslation();
  const defaultValue = { open: false, data: null, type: null };
  const [dialog, setDialog] =
    useState<Components.IDialog<Income.IIncome>>(defaultValue);

  const { handleOrderChange, orderBy } = useSortableFilter("sortedId");
  ...
  return (
    ...
    >
      ...
      {data?.length ? (
        <Table className="w-full bg-charcoal rounded-lg text-center mt-10">
        ...
        <EmptyWindow
          title={t("Bo’sh oyna")}
          description={t("Bu yerda daromadlar ro'yxatini ko'rishingiz mumkin")}
        />
      )}
